...
def split_handler( df): # , q1, q2, q3, q4
    sents_split = []
    true_x_sents_split = []
    true_y_sents_split = []
    false_x_sents_split = []
    ...
    # q1.put([json.dumps(sents_split)])
    # q2.put([json.dumps(y_sents_split)])
    # q3.put([json.dumps(total_y_sents_split)])
    # q4.put([json.dumps(sent_indexes_split)])
...
def sequence_handler( row):
    ...
    processor.add_keywords_from_list(keyword_list = true_keywords)
    processor.add_keywords_from_list(keyword_list= false_keywords)
    ...
    sents_list = [[str(word) for word in s] for s in sents]
